...
/*! \class CEpos2
	...
	\warning Units are in default (position:[qc](=4*encoder counts/revolution),
...
public:
    ...
    /** Constructor */
    CEpos2(const std::string& _port = "/dev/ttyS0", unsigned _baud = 115200, int8_t _nodeId = 0x00);
     ...
     *  \b Remarks
     *  The resolution of the short time velocity measurement (Velocity actual value,
	 *  Velocity sensor actual value) is dependent on the encoder pulse number (Sensor
	 *  Configuration) and the velocity measurement method (Miscellaneous Configuration
	 *   bit 3). To improve the short time velocity measurement resolution set the
	 *  Miscellaneous Configuration bit 3 to 1 or use an encoder with higher resolution.
     *  For example the short time velocity resolution with a 500-pulse encoder and
	 *  Miscellaneous Configuration Bit 3 = 0 is: 1 quadcount / ms = 60’000 / (4 * 500) = 30 rpm.
     *
     *  [qc/s]
     *
     ...
     *
     *  Velocity demand value is generated by profile generator and is the set value for
     ...
     *
     *  \b Description
     *  The velocity actual value is coupled to the velocity used as input to velocity
	 *  controller [Velocity units].
     ...
     *
     *
     *  The current actual value averaged [mA] represents the current actual value filtered
	 *  by 1st order digital lowpass filter with a cut-off frequency of 50 Hz.This function
	 *  reads the current of the motor averaged [mA].
     *
    ...
    void getDigOutFunc(uint16_t& state, uint16_t& mask, uint16_t& polarity);
...
private:
    ...
    /** \brief Function to read an object serial port
     *
    ...
    /** \brief Function send a frame to serial port
     *
