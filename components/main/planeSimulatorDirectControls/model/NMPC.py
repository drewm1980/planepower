...
#    dw = 0.
    ...
    U0 = np.array([append(U00[0,:2],zeros(2))])
    ...
    k1 = h * RotModelCarouselR(X_0,U,P,Scaling,"")
    k2 = h * RotModelCarouselR(X_0+k1/2.,U,P,Scaling,"")
    k3 = h * RotModelCarouselR(X_0+k2/2.,U,P,Scaling,"")
    k4 = h * RotModelCarouselR(X_0+k3,U,P,Scaling,"")
    
    X_1 = X_0 + 1./6. * ( k1 + 2*k2 + 2*k3 + k4 )
    
    # The state derivative (needed for the acceleration)
    dX_1 = RotModelCarouselR(X_1,U,P,Scaling,"")
    Xa_1 = vertcat([X_1,dX_1[3:6]])
    #Ifcn = SXFunction([X_0,U,h],[X_1, dX_1[3:6]])
    Ifcn = SXFunction([X_0,U,h],[Xa_1])
    Ifcn.init()
    generateSimpleCode(Ifcn,"rk4_direct_controls.h",
                      docstring="// This function integrates a kite model. See NMPC.py")

    # Done generating code.  Rest of the code ~should go in a separate file
    import sys
    sys.exit(0)
