...
% state = [x;y;z;dx;dy;dz;R(1,:);R(2,:);R(3,:);w1;w2;w3;r;dr;delta;ddelta];
state = [ 0.08271745,  2.2620972 ,  0.        , -7.10658795,  0.25986452,   0.        , -0.99351977, -0.07240986,  0.08760873,  0.10494709,   -0.28846895,  0.95172043, -0.04364155,  0.95474734,  0.29419881,    -0.13710396,  2.99942722,  0.92425281,  1.18      ,  0.        ,        0.        ,  3.14159265];
...
%     t   ddr dddelta  ua         ue
tu = [0,   0, 0, -0.00042919, -0.01271043;
      TP,  0, 0, -0.00042919, -0.01271043;];
...
Out = Model_integ_ACADO_rotation(0,X0,'const',P);
ConstCheck = Out(1:2)
% X = [x;y;z];
% Xc = X + R*XTE;
...
tic
[time,state]=ode15s('Model_integ_ACADO_rotation',[0 P.Tf],X0,intoptions,P);
ET = toc;
RelTime = num2str(P.Tf/ET);
display(['Simulation time ratio: ',RelTime]);

for k = 1:length(time)
    Forces(:,k) = Model_integ_ACADO_rotation(time(k),state(k,:),'output',P);
end
Out = [];
for k = 1:length(time)
    Out(:,k) = Model_integ_ACADO_rotation(time(k),state(k,:),'const',P);
end
...
for k = 1:length(labels)
    subplot(2,2,k)
    plot(time,state(:,15+k),'linewidth',2);
    title(labels{k})
end
