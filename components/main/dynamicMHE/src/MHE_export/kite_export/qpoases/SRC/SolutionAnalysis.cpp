...
returnValue SolutionAnalysis::getHessianInverse( QProblem* qp, real_t* hessianInverse )
{
	...
	/* Ask for the numnber of free and fixed variables, assumes that active set
	 * is constant for the variance-covariance evaluation */
	nFR = qp->getNFR( );
	nFX = qp->getNFX( );
	nAC = qp->getNAC( );
	...
	/* Calculate
	 * 
	 *  [ ("ACTIVE" KKT-MATRIX OF THE QP)^(-1) * g_b_bA_VAR ]^T
	 *
	 * The evaluation of the inverse of the KKT matrix of the QP with
	 * respect to the current active set uses the existing Cholesky
	 * and TQ-decompositions. For details cf. the (protected) 
	 * function determineStepDirection
	 */
	
	for( run3 = 0; run3 < NVMAX; run3++ )
	{
		/* Line wise loading of the input variance-covariance: */	
		/* i.e. select the corresponding elementary vector: */
...
returnValue SolutionAnalysis::getHessianInverse( QProblemB* qp, real_t* hessianInverse )
{
	...
	/* Calculate:
	 * 
	 *  [ ("ACTIVE" KKT-MATRIX OF THE QP)^(-1) * g_b_VAR ]^T
	 *
	 * The evaluation of the inverse of the KKT matrix of the QP with
	 * respect to the current active set uses the existing Cholesky
	 * and TQ-decompositions. For details cf. the (protected) 
