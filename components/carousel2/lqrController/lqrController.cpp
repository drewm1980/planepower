...
bool LqrController::configureHook()
{
	...
	{
		log(Error) << "lqrController: Cannot configure; carouselSimulator needs to be a peer!" << endlog();
		return false;
	}
	...
	{
		log(Info) << "lqrController: First read to measurements port was indeed not newData!" << endlog();
	}	
...
bool  LqrController::startHook()
{
	...
	{
		log(Error) << "lqrController: Cannot start without reference elevation!" << endlog();
		return false;
	}	
...
void  LqrController::updateHook()
{
	...
	if(!trigger_last_is_valid)
	{
		...
		log(Info) << "lqrController: returning early because need to initialize dt" << endlog();
		return;
	}
	...
	if (measurementStatus != NewData) 
	{
		log(Warning) << "lqrController: No new measurment data!! " << endlog();
		return;
	}	
	...
	{
		//log(Warning) << "lqrController: No new reference data on the input port!" << endlog();
	...
	{
		log(Warning) << "lqrController: Line angle sensor is out of range of the lookup table so cannot look up a reference speed!" << endlog();
		return;
	}
	...
	if(freezeFeedForwardTerm)
	{
		...
		{
			log(Error) << "lqrController: updateHook has been called before feedForwardTerm has had a chance to be set, while freezeFeedForwardTerm is set!" << endlog();
		}
	}
	else 
	{
	...
	double lqrTerm  = pTerm + iTerm + dTerm;

#define USE_MORITZ_IDEA 1
#if USE_MORITZ_IDEA	
	double lqrControlAsSpeed = lookup_steady_state_speed(feedForwardTermAsAngle+lqrTerm);
	if (isnan(lqrControlAsSpeed))
	{
		log(Warning) << "lqrController: Control (as elevation) is out of range of the lookup table so cannot look up the Control (as a speed)!" << endlog();
		return;
	}
	double controlAsSpeed = lqrControlAsSpeed; // Rad/s
#else
	double controlAsSpeed = feedForwardTermAsSpeed + lqrTerm; // Rad/s
#endif
...
void  LqrController::errorHook()
{}

ORO_CREATE_COMPONENT( LqrController )
