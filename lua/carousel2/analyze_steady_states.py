...
""")
    ...
    fileLocation = '../../components/carousel2/controllerTemplate/'
    filenameroot = 'steady_state_lookup_tables'
    boilerplate = """// Warning!!! This file was code generated by analyze_steady_states.py!
...
"""
    ...
    fd = open(fileLocation+filenameroot+'.cpp','w')
    fd.write(fileContents)
    fd.close()

    # Codegen a header
    h_speed = lookup_template_h.substitute(domain='elevation',raange='speed')
    h_elevation = lookup_template_h.substitute(domain='speed',raange='elevation')
    fileContents = h_speed + h_elevation 
    fd = open(fileLocation+filenameroot+'.h','w')
    fd.write(fileContents)
    fd.close()
